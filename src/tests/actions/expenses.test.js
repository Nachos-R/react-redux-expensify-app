...
test('should edit expenses from firebase', (done) => {
    ...
    const updates = {description: 'this is description'}
    store.dispatch(startEditExpense(id, updates)).then(() => {
        ...
        return database.ref(`users/${uid}/expenses/${id}`).once('value').then((snapshot) => {
            expect(snapshot.val().description).toBe('this is description');
            done();
        });
    });
});
...
test('should setup the expenses from firebase', (done) => {
    const store = createMockStore(defaultAuthState);
